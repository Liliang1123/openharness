...
- Affected test scope: the correction-only derivatives of the two named Agent Runtime tests and governance regression coverage for clean/dirty candidate reproducibility.
...
Implementation MUST NOT begin until this proposal, its design, tasks, and governance spec delta are explicitly approved. Approval must cover the selected extraction strategy and the new governance change id `refactor-gate-closure-evidence-scope`. Before staging or committing the new scope, obtain a fresh confirmation because the file scope and evidence contract have changed.
