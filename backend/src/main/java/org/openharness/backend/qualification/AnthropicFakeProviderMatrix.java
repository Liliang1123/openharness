...
public final class AnthropicFakeProviderMatrix {
  ...
  public Map<String, Object> run(String baseUrl, String apiKey) {
    ...
    ProviderConfig config = new ProviderConfig(
        "fake-anthropic", "anthropic", baseUrl, apiKey,
    ...
    rows.add(runRow("structured-tool", true, "tool_use", toolReq, config, adapter, () -> adapter.chat(toolReq, config)));
  ...
  private Map<String, Object> runRow(String id, boolean required, String capability, ModelChatRequest request, ProviderConfig config, AnthropicAdapter adapter, java.util.function.Supplier<ModelChatResponse> action) {
    ...
    try {
      ...
      if ("sync".equals(id)) {
        ...
        boolean pass = resp.usage().promptTokens() == 9 && resp.usage().completionTokens() == 4
            && resp.usage().totalTokens() == 13 && resp.usage().cacheReadTokens() == 2
            && resp.usage().cacheWriteTokens() == 1;
        result = pass ? "pass" : "fail";
        observed.put("promptTokens", resp.usage().promptTokens());
        observed.put("completionTokens", resp.usage().completionTokens());
        observed.put("cacheReadTokens", resp.usage().cacheReadTokens());
        observed.put("cacheWriteTokens", resp.usage().cacheWriteTokens());
        ...
        boolean reasoningPassed = resp.message().reasoningBlocks() != null
            ...
            && "sig".equals(resp.message().reasoningBlocks().get(0).get("signature"));
        result = reasoningPassed ? "pass" : "fail";
        observed.put("reasoningBlocks", resp.message().reasoningBlocks());
      }
    } catch (Exception e) {
      if ("terminal-error".equals(id)) {
        boolean terminalObserved = e.getMessage().contains("Anthropic error 400");
  ...
  private Map<String, Object> buildOracle(String id) {
    ...
    if ("exact-usage".equals(id)) return Map.of("promptTokens", 9, "completionTokens", 4, "cacheReadTokens", 2, "cacheWriteTokens", 1);
