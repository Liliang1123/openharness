...
| Gate C — real integrations | authorized Codex OAuth six-row production report + no-overwrite provider decision, Java sandbox and MCP matrix reports, OAuth/secret negative scans; API-key Provider reports remain advisory | Required model/tool/security evidence before Stage 3; API-key FAIL/BLOCKED does not veto global model-regression PASS |
...
- Maintain the [Agent Runtime v1 production runbook](file:///Users/elvis/file/develop/opensource/openharness/.worktrees/add-openclacky-runtime-parity-roadmap/docs/architecture/agent-runtime-v1-production-runbook.md) and [qualification report templates](file:///Users/elvis/file/develop/opensource/openharness/.worktrees/add-openclacky-runtime-parity-roadmap/docs/verification/agent-runtime-v1/).
...
### Task 10: Codex OAuth Required Track And API-Key Advisory Matrices

**Gate:** every real model call requires its own explicit authorization. Reconciliation of existing immutable evidence is local/read-only and makes no model call.

**Gate C required family (amended 2026-07-15 via approved `adopt-codex-oauth-regression-qualification`):** official local Codex CLI/app-server + ChatGPT/Codex OAuth only. Required production rows are `codex-real-sync`, `codex-real-reasoning`, `codex-real-usage`, `codex-real-stream`, `codex-real-cancellation`, and `codex-real-redaction`; all must be required/PASS with current immutable report/client source binding, authorization metadata, redaction, and no mock/API-key fallback.

**API-key Provider authority:** Zhipu, generic OpenAI-compatible, Anthropic, and future API-key real matrices remain supported and truthful but are advisory for global Runtime qualification. Their rows keep observed PASS/FAIL/BLOCKED and their reports may remain provider-internally strict; missing/expired credentials or a blocked provider-specific oracle does not veto global model-regression PASS. A Provider-specific production-qualified claim still requires that Provider's dedicated real matrix PASS.

**Required decision reconciliation:** use the preflight-approved `qualification:gate-c-provider` CLI from [the 2026-07-15 implementation plan](file:///Users/elvis/file/develop/opensource/openharness/.worktrees/add-openclacky-runtime-parity-roadmap/docs/superpowers/plans/2026-07-15-adopt-codex-oauth-regression-qualification.md). It reads the immutable [Codex production report](file:///Users/elvis/file/develop/opensource/openharness/.worktrees/add-openclacky-runtime-parity-roadmap/docs/verification/agent-runtime-v1/providers/2026-07-12-codex-app-server-production.json), current [Codex client source](file:///Users/elvis/file/develop/opensource/openharness/.worktrees/add-openclacky-runtime-parity-roadmap/backend/src/main/java/org/openharness/backend/service/provider/CodexAppServerClient.java), and any advisory reports; it writes exactly one new `0600` decision artifact with `wx`, never reads OAuth/API-key credentials, and never invokes a model. Source/report hash mismatch, required-row mismatch, secret scan hit, or existing target stops reconciliation.

The Java API-key qualification harness below is retained as advisory compatibility infrastructure. Its strict per-report fixed-row contract is unchanged and is not Codex evidence.
...
**Fixed API-key production rows:** every provider-specific advisory report contains exactly these provider-prefixed row ids in this order: `sync`, `stream`, `single-tool-call`, `multi-step-tool-call`, `structured-arguments`, `reasoning`, `usage`, `cost`, `503-retry`, `timeout`, `cancellation`, `terminal-error`, `redaction`. Within that Provider's own report no row is optional: missing/duplicate/reordered/FAIL/BLOCKED rows prevent that report from claiming provider-specific PASS. The entire report is advisory to global Runtime qualification and cannot veto the required Codex decision. Unsupported capabilities remain `blocked`, never skipped or rewritten PASS. Each row retains its production track, sanitized environment, protocol/model, capability prerequisites, request hash, observed sequence, oracle, usage/cost, duration, and result.
...
- [x] **Step 18: Run one explicitly authorized OpenAI-compatible advisory production attempt.** Use the exact authorized values with `--real --provider openai-compatible`; inject `OPENAI_COMPATIBLE_API_KEY` only into that process, leave `ANTHROPIC_API_KEY` unset, and choose a new report path under [provider evidence](file:///Users/elvis/file/develop/opensource/openharness/.worktrees/add-openclacky-runtime-parity-roadmap/docs/verification/agent-runtime-v1/providers/). The immutable 2026-07-14 Zhipu/glm-4-flash report records 5 PASS and 8 BLOCKED rows; it remains advisory and does not close required Codex task 3.1. Immediately scan captured stdout/stderr/report/temp directory for the fixed canaries and credential SHA-256 supplied out-of-band; any hit is a hard FAIL requiring artifact isolation.
- [ ] **Step 19: Run an Anthropic advisory production qualification only after separate authorization.** Use `--real --provider anthropic`, the authorized endpoint/model/pricing/budget/new report path, fixed protocol version `2023-06-01`, and only `ANTHROPIC_API_KEY`. Preserve/cache-account `cache_read_input_tokens` and `cache_creation_input_tokens`. Missing credentials or any BLOCKED/FAIL remains advisory to global Runtime qualification, while provider-specific Anthropic promotion stays unavailable until its own matrix passes.
- [x] **Step 20: Independently review API-key real evidence before any provider-specific claim.** Verify report SHA-256, exact fixed rows, production track, endpoint/environment fingerprint, model/protocol version, request hashes, observed sequences, usage/cost, timeout/cancel cleanup, per-report required-row veto, and negative secret scans. Review PASS may qualify only that API protocol; it is not required Codex evidence and does not veto global model-regression PASS.
- [x] **Step 21: Generate and review the required Codex provider-decision artifact.** Run the no-overwrite CLI against the authorized Codex report/current client source and advisory API reports, prove a second call cannot overwrite the target, run strict sensitive-data scans, and check OpenSpec 3.1 only after the decision and implementation Review PASS.

**Formal commands:** Step 16 is the mandatory no-network API-runner implementation gate. Steps 18 and 19 are separately authorized advisory production executions and MUST NOT be folded into tests, CI, preflight, or implementation verification. Step 21 is local evidence reconciliation only and makes no model call. Dashboard remains `proposed`; API-key advisory FAIL/BLOCKED never changes required Codex or non-model gate status.
...
**Production executor plan:** [2026-07-15-agent-runtime-gate-d-production-executor.md](file:///Users/elvis/file/develop/opensource/openharness/.worktrees/add-openclacky-runtime-parity-roadmap/docs/superpowers/plans/2026-07-15-agent-runtime-gate-d-production-executor.md). The current pure harness is not a production caller: implementation and strict Review of the supervisor/worker/probe/journal slice MUST reach `preflight_ready` before the start-approval checklist below can pass.

- [x] Implement and strictly review the production Gate D supervisor, real Runtime child-process restarts, 20-worker deterministic load, active probes, append-only journal, and no-overwrite partial/final report writer without starting the formal run. `preflight_ready` evidence: [2026-07-15-preflight-ready.md](file:///Users/elvis/file/develop/opensource/openharness/.worktrees/add-openclacky-runtime-parity-roadmap/docs/verification/agent-runtime-v1/gate-d/2026-07-15-preflight-ready.md); implementation Review: [2026-07-15-agent-runtime-gate-d-production-executor-implementation-review.md](file:///Users/elvis/file/develop/opensource/openharness/.worktrees/add-openclacky-runtime-parity-roadmap/docs/review/2026-07-15-agent-runtime-gate-d-production-executor-implementation-review.md).
