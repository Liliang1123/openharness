...
interface RuntimeStorageHealthSample {
  ready: boolean;
  reason: string | null;
  mainThreadHeartbeatMs: number;
}
...
async function runRegression(input: CliInput, initialState: BoundState): Promise<number> {
  ...
  const storageHealthSamples: RuntimeStorageHealthSample[] = [];
  ...
  try {
    ...
    for (let sampleIndex = 0; sampleIndex < REQUIRED_SAMPLES; sampleIndex += 1) {
      ...
      let storageHealth: RuntimeStorageHealthSample = {
        ready: false,
        reason: "RUNTIME_READINESS_PROBE_FAILURE",
        mainThreadHeartbeatMs: 0
      };
      try {
        storageHealth = await probeRuntimeStorageHealth(runtimeUrl, input.serviceToken);
        storageHealthSamples.push(storageHealth);
        if (!storageHealth.ready) {
          hardFailures.add("RUNTIME_READINESS_DEGRADED");
        }
        if (
          storageHealth.reason === "RUNTIME_STORAGE_QUEUE_FULL"
          || storageHealth.reason === "RUNTIME_STORAGE_UNAVAILABLE"
        ) {
          hardFailures.add(storageHealth.reason);
        }
      } catch {
      ...
      const checkpoint = {
        ...
        storageWorkerReady: storageHealth.ready,
        storageWorkerReason: storageHealth.reason,
        mainThreadHeartbeatMs: storageHealth.mainThreadHeartbeatMs,
  ...
  const report = createRuntimeBaselineReport({
    ...
    environment: {
      ...
      storageWorkerMetrics: summarizeStorageHealth(storageHealthSamples),
      qualification
    },
    thresholds: {
      ...DEFAULT_RUNTIME_BASELINE_THRESHOLDS,
...
async function probeRuntimeStorageHealth(
  runtimeUrl: string,
  serviceToken: string
): Promise<RuntimeStorageHealthSample> {
  ...
  const mainThreadHeartbeatMs = Math.max(0, performance.now() - startedAt);
  const body = await response.json() as {
    status?: unknown;
    reason?: unknown;
  };
  return {
    ready: response.ok && body.status === "ready",
    reason: typeof body.reason === "string" ? body.reason : null,
    mainThreadHeartbeatMs
  };
}

function summarizeStorageHealth(
  samples: readonly RuntimeStorageHealthSample[]
): {
  samples: number;
  workerUnavailableSamples: number;
  queueFullSamples: number;
  heartbeatMedianMs: number | null;
  heartbeatMaxMs: number | null;
} {
  const heartbeat = samples.map(sample => sample.mainThreadHeartbeatMs);
  return {
    samples: samples.length,
    workerUnavailableSamples: samples.filter(
      sample => sample.reason === "RUNTIME_STORAGE_UNAVAILABLE"
    ).length,
    queueFullSamples: samples.filter(
      sample => sample.reason === "RUNTIME_STORAGE_QUEUE_FULL"
    ).length,
    heartbeatMedianMs: median(heartbeat),
    heartbeatMaxMs: heartbeat.length > 0 ? Math.max(...heartbeat) : null
  };
}
