...
describe("stable history layering", () => {
  ...
  it("rejects runtime sentinel writes at the HistoryStore boundary", () => {
    ...
    expect(() => store.append("t1", "c1", sentinel("PENDING_APPROVAL"))).toThrow(/stable history/i);
    expect(() => store.append("t1", "c1", sentinel("POLICY_DENY"))).toThrow(/stable history/i);

    store.append("t1", "c1", { role: "user", content: "stable" });
    expect(store.get("t1", "c1")).toEqual([{ role: "user", content: "stable" }]);
  });

  it("filters legacy runtime sentinels when reading persisted history", async () => {
    const tenantDir = join(TEST_DIR, "t1");
    mkdirSync(tenantDir, { recursive: true });
    writeFileSync(join(tenantDir, "c1.json"), JSON.stringify({
      tenantId: "t1",
      conversationId: "c1",
    ...
    expect(store.get("t1", "c1")).toEqual([
      { role: "user", content: "hello" },
      { role: "assistant", content: "world" }
    ]);
    await expect(store.list("t1")).resolves.toEqual([
