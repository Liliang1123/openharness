...
const singletonFlags: ReadonlyMap<string, keyof Omit<ParsedArguments, "advisoryReports">> = new Map([
  ...
  ["--codex-client-source", "codexClientSource"],
  ["--output", "output"],
...
function parseArguments(args: string[]): ParsedArguments {
  ...
  if (!values.projectRoot || !values.codexReport || !values.codexClientSource || !values.output || !values.generatedAt) {
    throw new CliError("invalid_arguments");
  }
...
function resolveInput(root: string, value: string): { absolute: string; projectRelative: string } {
  try {
    ...
    if (!isInside(root, candidate) || !statSync(candidate).isFile()) {
...
function resolveOutput(root: string, value: string): string {
  try {
    ...
    if (!isInside(root, canonicalCandidate)) {
...
export function runGateCProviderReconciliation(args: string[], io: CliIo = {
  ...
  try {
    ...
    if (existsSync(outputPath)) {
      throw new CliError("output_exists");
    }

    const codexPath = resolveInput(root, parsed.codexReport);
    const clientSourcePath = resolveInput(root, parsed.codexClientSource);
    const advisoryPaths = parsed.advisoryReports.map((path) => resolveInput(root, path));
    const codex = readReport(codexPath.absolute, codexPath.projectRelative);
    const advisory = advisoryPaths.map((path) => readReport(path.absolute, path.projectRelative));
    const codexClientSourceSha256 = sha256(readFileSync(clientSourcePath.absolute));
    const decision = evaluateGateCProviderQualification({
      generatedAt: parsed.generatedAt,
      codex,
      codexClientSourceSha256,
      advisory
    });
    const bytes = `${JSON.stringify(GateCProviderDecisionSchema.parse(decision), null, 2)}\n`;
    ...
    if (decision.result === "blocked") {
      io.stderr(`result=blocked policy=codex-oauth-required-v1 blockerCount=${decision.blockers.length} blockerClasses=${decision.blockers.join(",")}`);
      return 3;
    }
    io.stdout(`result=pass policy=codex-oauth-required-v1 output=${basename(outputPath)} sha256=${sha256(bytes)}`);
    return 0;
  } catch (error) {
